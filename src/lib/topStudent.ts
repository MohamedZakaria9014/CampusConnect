...
export function evaluateTopStudentStatus(
  ...
  if (gpa >= criteria.minGPA) {
    qualifyingReasons.push(
      `Academic Excellence (GPA: ${gpa.toFixed(2)} / 4.0)`,
    );
  }
  ...
  if (helpfulAnswers >= criteria.minHelpfulAnswers) {
    qualifyingReasons.push(
      `Helpful Answers (${helpfulAnswers} Upvoted Solutions)`,
    );
  }
  ...
  return {
    ...
    progressToNext: {
      ...
      repProgress: Math.min(
        100,
        Math.round((reputation / criteria.minReputation) * 100),
      ),
      helpfulProgress: Math.min(
        100,
        Math.round((helpfulAnswers / criteria.minHelpfulAnswers) * 100),
      ),
      bestAnswersProgress: Math.min(
        100,
        Math.round((bestAnswers / criteria.minBestAnswers) * 100),
      ),
    },
  };
}
