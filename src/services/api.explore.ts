...
export async function fetchMajors(): Promise<Major[]> {
  try {
    ...
    console.warn(
      "Could not fetch majors from Supabase, using local constants fallback:",
      err,
    );
  }
...
export async function fetchCourses(universityId?: string): Promise<Course[]> {
  ...
  try {
    let query = supabase
      .from("posts")
      .select("course_code, university_id")
    ...
    const uniqueCodes = Array.from(
      new Set(data.map((p) => p.course_code).filter(Boolean)),
    ) as string[];
...
export async function searchStudents(queryStr: string): Promise<Profile[]> {
  const { data, error } = await supabase
    ...
    .or(
      `full_name.ilike.%${queryStr}%,username.ilike.%${queryStr}%,major.ilike.%${queryStr}%`,
    )
    .limit(20);
...
export async function fetchTrendingTopics(): Promise<TrendingTopic[]> {
  try {
    const { data: posts, error } = await supabase
      .from("posts")
      .select(
        "id, title, category, course_code, upvotes_count, answers_count, views_count, created_at, profiles(full_name, is_top_student, reputation)",
      )
      // Pre-sort at DB level so we get the best candidates — JS re-ranks and returns top 8
    ...
    const ranked = posts.map((p: any) => {
      ...
      const authorProfile = Array.isArray(p.profiles)
        ? p.profiles[0]
        : p.profiles;
