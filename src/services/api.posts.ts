...
export interface FetchPostsParams {
  ...
  /** Pre-resolved university IDs from the caller's cache — avoids a sequential DB round-trip */
...
export async function fetchPosts(
  ...
  if (subject) {
    ...
    if (
      lowerS.includes("computer") ||
      lowerS.includes("software") ||
      lowerS.includes("cyber")
    ) {
      ...
      lowerS.includes("med") ||
      lowerS.includes("pharm") ||
      lowerS.includes("dent") ||
      lowerS.includes("nurs")
    ) {
      conditions.push("category.ilike.%Medicine%");
    } else if (
      lowerS.includes("business") ||
      lowerS.includes("fin") ||
      lowerS.includes("econ") ||
      lowerS.includes("mkt")
    ) {
  ...
  if (searchQuery) {
    ...
    // Use pre-resolved IDs passed from caller's cache — no extra DB round-trip
    const resolvedUniIds = universityIds ?? [];

    if (resolvedUniIds.length > 0) {
      query = query.or(
        `title.ilike.%${q}%,content.ilike.%${q}%,category.ilike.%${q}%,course_code.ilike.%${q}%,code_language.ilike.%${q}%,university_id.in.(${resolvedUniIds.join(",")})`,
      );
    } else {
      query = query.or(
...
export async function createPost(postData: Partial<Post>): Promise<Post> {
  if (
    ...
    throw new Error(
      "Missing required fields for post (author_id, title, content, category)",
    );
  }
