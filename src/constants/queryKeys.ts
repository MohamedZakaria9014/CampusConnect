...
export const queryKeys = {
  posts: {
    ...
    list: (category?: string, filter?: string, userId?: string, universityId?: string) =>
      [...queryKeys.posts.lists(), { category, filter, userId, universityId }] as const,
    ...
    search: (query: string) =>
      [...queryKeys.posts.searches(), query] as const,
  ...
  users: {
    ...
    following: (targetId: string, currentUserId?: string) =>
      [...queryKeys.users.all, 'following', targetId, { currentUserId }] as const,
  },
  notifications: {
