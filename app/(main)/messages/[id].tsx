...
import { ArrowLeft, Send, Image as ImageIcon, Code, X, Trash2 } from 'lucide-react-native';
...
export default function ChatRoomScreen() {
  ...
  const [otherMemberLastReadAt, setOtherMemberLastReadAt] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    findOrCreateDirectConversation(user.id, id, postId).then(async (conversation) => {
      ...
      channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (updated && updated.user_id !== user.id && updated.last_read_at) {
  ...
  const handleSendMessage = async () => {
    ...
    if (!activeConvId) {
      Alert.alert('Connecting', 'Connecting to chat... Please try again in a moment.');
      return;
    }
    ...
    try {
      ...
      if (localImage) {
        publicImageUrl = await uploadImageToSupabase(localImage, 'posts', 'chat_images');
      }
      ...
      setMessages((prev) => {
        ...
        return prev.map((m) => (m.id === tempId ? { ...sent, status: 'delivered' } : m));
      });
      setTimeout(() => flatListRef.current?.scrollToEnd({ animated: true }), 100);
    } catch (e: any) {
      console.warn('Failed to send message:', e);
      setMessages((prev) =>
        prev.map((m) => (m.id === tempId ? { ...m, is_pending: false, has_error: true } : m))
      );
      Alert.alert('Error', 'Failed to send message. Please check your network connection.');
    } finally {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        {/* Top Header */}
        <View style={[styles.topHeader, { borderColor: colors.border, backgroundColor: colors.surface }]}>
          <TouchableOpacity
            onPress={() => {
              if (user?.id) {
                queryClient.invalidateQueries({ queryKey: queryKeys.chat.conversations(user.id) });
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Avatar url={targetUser?.avatar_url} name={targetUser?.full_name || 'Student'} size={40} isOnline />
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            const otherReadTime = otherMemberLastReadAt ? new Date(otherMemberLastReadAt).getTime() : 0;
        ...
        <View style={[styles.inputContainer, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <TouchableOpacity
            ...
            disabled={isSending || (!inputContent.trim() && !codeSnippet.trim() && !attachedImageUri)}
            style={[
              styles.sendBtn,
              {
                backgroundColor:
                  inputContent.trim() || codeSnippet.trim() || attachedImageUri ? colors.primary : colors.border,
              },
            ]}
          >
