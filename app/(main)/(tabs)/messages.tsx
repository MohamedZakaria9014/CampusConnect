...
export default function ConversationsListScreen() {
  ...
  const handleDeleteConversation = useCallback(
    (item: Conversation) => {
      const otherMember =
        item.members?.find((m) => m.id !== user?.id) || item.members?.[0];
              ...
              try {
                ...
                if (item.unread_count && item.unread_count > 0) {
                  ...
                  useChatStore
                    .getState()
                    .setUnreadCount(
                      Math.max(0, currentUnread - item.unread_count),
                    );
                }
      ...
      return (
        <TouchableOpacity
          ...
          onPress={() => {
            if (item.unread_count && item.unread_count > 0) {
              ...
              useChatStore
                .getState()
                .setUnreadCount(Math.max(0, currentUnread - item.unread_count));
              item.unread_count = 0;
            }
            queryClient.setQueryData(
              queryKeys.chat.conversations(user?.id),
        ...
        >
          <Avatar
            ...
            name={otherMember?.full_name || "Student"}
            size={52}
            isOnline
          />
          ...
          <View style={styles.convMeta}>
            ...
            <Text
              style={[styles.lastMsgText, { color: colors.textSecondary }]}
              numberOfLines={1}
            >
