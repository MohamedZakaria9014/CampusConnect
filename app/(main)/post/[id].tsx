    ...
    try {
      ...
      const uploadedUrls =
        answerLocalImages.length > 0
          ? await Promise.all(
              answerLocalImages.map((uri) =>
                uploadImageToSupabase(uri, "posts", "answer_images"),
              ),
            )
          : undefined;
      ...
      >
        ...
        >
          ...
          >
            <View style={styles.authorHeader}>
              ...
              <View style={styles.authorMeta}>
                ...
                <Text style={[styles.subMeta, { color: colors.textSecondary }]}>
                  ...
                  {author?.university?.short_name || author?.university?.name
                    ? ` · ${author.university.short_name || author.university.name}`
                    : ""}
            ...
            <View style={styles.pillRow}>
              {post.course_code && (
                ...
                >
                  <Text
                    style={[styles.coursePillText, { color: colors.primary }]}
                  >
                    {post.course_code}
                  </Text>
                </View>
              ...
              {post.is_solved && (
                <View
                  style={[styles.solvedPill, { backgroundColor: "#10B98120" }]}
                >
          ...
          {answers && answers.length > 0 ? (
            ...
            <View style={styles.noAnswersBox}>
              ...
              <Text
                style={[styles.noAnswersText, { color: colors.textSecondary }]}
              >
                No answers yet. Be the first to help{" "}
                {author?.full_name || "this student"}!
              </Text>
            </View>
          )}
        </ScrollView>
        ...
        >
          ...
          <View style={styles.inputRow}>
            ...
            <TouchableOpacity
              onPress={handlePickAnswerImage}
              style={styles.codeToggleBtn}
            >
              <ImageIcon
                size={20}
                color={
                  answerLocalImages.length > 0 ? colors.primary : colors.icon
                }
              />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              disabled={
                isSubmitting ||
                (!answerText.trim() && answerLocalImages.length === 0)
              }
              style={[
                styles.sendBtn,
                {
